...
describe("memory-formation", () => {
    ...
    const prisma = {
      ...
      userMemoryNote: {
        ...
        update: async (args: { where: { id: string }; data: Record<string, unknown> }) => {
          const existing = userMemories.find((item) => item.id === args.where.id);
          if (!existing) {
            throw new Error("user update target missing");
          }
  ...
  it("turns semantically duplicated ADD actions into updates instead of creating a second note", async () => {
    const userMemories: Array<Record<string, unknown>> = [
      {
        id: "note-1",
        _key: "guild-1:user-1:no-spam-pings",
        guildId: "guild-1",
        userId: "user-1",
        key: "no-spam-pings",
        value: "Не пинговать пользователя без причины.",
        active: true,
      }
    ];
    ...
    const llm = {
      chat: async () => {
        chatCall += 1;
        if (chatCall === 1) {
          return { message: { role: "assistant" as const, content: "Пользователь просит не пинговать его без причины и не спамить лишний раз." } };
        }
        if (chatCall === 2) {
          return { message: { role: "assistant" as const, content: JSON.stringify({ facts: ["Пользователь не любит лишние пинги и спам."] }) } };
        }
    ...
    expect(userMemories).toHaveLength(1);
    expect(String(userMemories[0].value)).toContain("не спамить");
  });

  it("deduplicates semantically duplicated actions within the same batch", async () => {
    const userMemories: Array<Record<string, unknown>> = [];
    ...
    const llm = {
      chat: async () => {
        chatCall += 1;
        if (chatCall === 1) {
          return { message: { role: "assistant" as const, content: "Пользователь просит не пинговать его без причины и не спамить." } };
        }
        if (chatCall === 2) {
          return { message: { role: "assistant" as const, content: JSON.stringify({ facts: ["Пользователь не любит лишние пинги и спам."] }) } };
        }
              ...
              actions: [
                {
                  ...
                  scope: "user",
                  key: "no-extra-pings",
                  text: "Не пинговать пользователя без причины.",
                },
                {
                  event: "ADD",
                  scope: "user",
    ...
    expect(result.actions).toEqual([
      ...
      expect.objectContaining({
        event: "NOOP",
        reason: expect.stringContaining("batch_semantic_duplicate:"),
      }),
    ]);
    expect(userMemories).toHaveLength(1);
    expect(String(userMemories[0].value)).toContain("не спамить");
